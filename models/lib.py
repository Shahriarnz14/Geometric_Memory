# Trying to stitch together a simpler version of Microsoft's phi-2
import math
...
def gelu(x):
    """A Copying this version of gelu that's used everywhere for some reason
...
class MLP(nn.Module):
    ...
    def __init__(self, config):
        ...
        super().__init__()
        self.config = config
        self.swapd = nn.Linear(config.n_embd, 4 * config.n_embd, bias=config.bias)
...
class Attention(nn.Module):
    ...
    def __init__(self, config, layer_idx, rotary=False):
        ...
        self.register_buffer(
            ...
            torch.tril(torch.ones(config.block_size, config.block_size)).view(
                1, config.block_size, config.block_size, 1
            ),
            persistent=False,
        )
        # else:
    ...
    def forward(self, x, cache=None, attn_mask=None):
        ...
        start_pos = (
            0 if cache is None or not cache.use_caching else cache.cur_seq_len[self.layer_idx]
        )
        ...
        # else:
        att = torch.einsum("bmhd,bnhd->bmnh", queries, keys)  # bsz x sl x s2 x n_heads
        # Make sure x is sl = 32 x n_heads
        if cache is None or not cache.use_caching:
            ...
            att = att.masked_fill(
                self.bias[:, seq_len - query_len : seq_len, :seq_len, :] == 0,
                float("-inf"),
            )

        # Apply the padding mask
        if attn_mask is not None:
            # attn_mask shape is (bsz, seq_len). Reshape for broadcasting to att shape
            # att shape: (bsz, query_len, seq_len, n_heads)
            mask = attn_mask.view(bsz, 1, seq_len, 1)
            att = att.masked_fill(mask == 0, float("-inf"))

        att = F.softmax(att / math.sqrt(self.head_dim), dim=2)
        out = torch.einsum("bmnh,bnhd->bmhd", att, values)  # bsz x sl x n_heads x head_dim

        out = self.proj(out.contiguous().view((bsz, query_len, self.n_embd)))  # bsz x sl x embd_dim
...
class RotaryEmbedding(nn.Module):
    ...
    def forward(self, queries, keys, start_pos=0):
        ...
        Args:
            queries: Input parameter.
            keys: Input parameter.
            start_pos: Input parameter.
