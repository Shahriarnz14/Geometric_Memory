...
from geometric_memory.models.config import PythiaConfig
from geometric_memory.models.lib import Attention, LayerNorm, MLP
from geometric_memory.utils.load import load_pythia
import torch
...
class Block(nn.Module):
    ...
    def forward(self, x, cache=None):
        ...
        x_attn = self.attn(self.ln_1(x), cache)
        x_mlp = self.mlp(self.ln_2(x))
        x = x_attn + x_mlp + residual

        return x


class Pythia(DeepSequenceModel):
    """Pythia definition.
    ...
    def from_pretrained(cls, model_type, teacherless_token=None):
        ...
        model_hf = GPTNeoXForCausalLM.from_pretrained(
            "EleutherAI/" + model_type, revision="step43000"
        )
